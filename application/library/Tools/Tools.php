...
class Tools
{
    ...
    public static function getBrowserInfo()
    {
        ...
        $info = array('type'=>$browser ,'vers' => $version);
    ...
    public static function returnAjaxJsonUnicode($array)
    {
        if (!headers_sent()) {
            header("Content-Type: application/json; charset=utf-8");
        }
    ...
    public static function curl($url, $method = 'GET', $postFields = null, $header = null)
    {
        ...
        $responseHeader = str_replace(array("\r\n", "\r", "\n"), ";", $responseHeader);

        NewLog::curlLog('Info:' . json_encode(curl_getinfo($ch)) . ' ResponseHeader:' . json_encode($responseHeader));
//        Log::getInstance('curl')->write('Info:' . json_encode(curl_getinfo($ch)) . ' ResponseHeader:' . json_encode($responseHeader));
...
//            throw new \Exception(curl_error($ch), 0);
     ...
     * @param null $headers
     */
    public static function curl_java($url, $method='Post', $postFields = null, $header = null)
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HEADER, 1);
        curl_setopt($ch, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0);
        ...
        $responseHeader = substr($response, 0, $headerSize);
        $responseHeader = str_replace(array("\r\n", "\r", "\n"), ";", $responseHeader);
        NewLog::curlLog('Info:' . json_encode(curl_getinfo($ch)) . ' ResponseHeader:' . $responseHeader);
//        Log::getInstance('curl')->write('Info:' . json_encode(curl_getinfo($ch)) . ' ResponseHeader:' . $responseHeader);
...
//            throw new \Exception(curl_error($ch), 0);
    ...
    public static function getYafEnviron()
    {
    ...
    {
        return self::getYafEnviron() == 'product';
    }

    public static function isDevelop()
    {
        return self::getYafEnviron() == 'develop';
    }

    public static function isTest()
    {
        return self::getYafEnviron() == 'test';
    }
    ...
    //获取一组时间戳的时间间隔
    public static function getInParkLongTime($start_time, $end_time)
    {
